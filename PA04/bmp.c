...
unsigned char addCheck(unsigned char a, char b);
...
BMP_Image *Convert_24_to_16_BMP_Image_with_Dithering(BMP_Image *image){
  ...
  unsigned char (*addr)[image->header.width * 3 + getPad(24, image->header.width)];
  addr = (unsigned char (*) [image->header.width * 3 + image->header.width % 4])&(image->data[0]);
  unsigned char (*ptr)[n_image->header.width * 2 + getPad(16, n_image->header.width)];
...
/*
  for (i = image->header.height - 1; i >= 0; i--){
    for (j = 0; j < image->header.width * 3; j = j+3){
      unsigned char qeB = addr[i][j] - (addr[i][j]/8.0 * 255 / 31.0);
      unsigned char qeG = addr[i][j+1] - (addr[i][j+1]/8.0 * 255 / 31.0);
      unsigned char qeR = addr[i][j+2] - (addr[i][j+2]/8.0 * 255 / 31.0);

      if (j+5 < image->header.width * 3){
        addr[i][j+3] = addr[i][j+3] + qeB * 7/16;
        addr[i][j+4] = addr[i][j+4] + qeG * 7/16;
        addr[i][j+5] = addr[i][j+5] + qeR * 7/16;
        if(addr[i][j+3] > 255){addr[i][j+3] = 255;}
        if(addr[i][j+3] < 0){addr[i][j+3] = 0;}
        if(addr[i][j+4] > 255){addr[i][j+4] = 255;}
        if(addr[i][j+4] < 0){addr[i][j+4] = 0;}
        if(addr[i][j+5] > 255){addr[i][j+5] = 255;}
        if(addr[i][j+5] < 0){addr[i][j+5] = 0;}
      }
      if (i-1 >= 0 && j-3 >= 0){
        addr[i-1][j-3] = addr[i-1][j-3] + qeB * 3/16;
        addr[i-1][j-2] = addr[i-1][j-2] + qeG * 3/16;
        addr[i-1][j-1] = addr[i-1][j-1] + qeR * 3/16;
        if(addr[i-1][j-3] > 255){addr[i-1][j-3] = 255;}
        if(addr[i-1][j-3] < 0){addr[i-1][j-3] = 0;}
        if(addr[i-1][j-2] > 255){addr[i-1][j-2] = 255;}
        if(addr[i-1][j-2] < 0){addr[i-1][j-2] = 0;}
        if(addr[i-1][j-1] > 255){addr[i-1][j-1] = 255;}
        if(addr[i-1][j-1] < 0){addr[i-1][j-1] = 0;}
      }
      if (i-1 >= 0){
        addr[i-1][j] = addr[i-1][j] + qeB * 5/16;
        addr[i-1][j+1] = addr[i-1][j+1] + qeG * 5/16;
        addr[i-1][j+2] = addr[i-1][j+2] + qeR * 5/16;
        if(addr[i-1][j] > 255){addr[i-1][j] = 255;}
        if(addr[i-1][j] < 0){addr[i-1][j] = 0;}
        if(addr[i-1][j+1] > 255){addr[i-1][j+1] = 255;}
        if(addr[i-1][j+1] < 0){addr[i-1][j+1] = 0;}
        if(addr[i-1][j+2] > 255){addr[i-1][j+2] = 255;}
        if(addr[i-1][j+2] < 0){addr[i-1][j+2] = 0;}
      }
      if (i-1 >= 0 && j+5 < image->header.width * 3){
        addr[i-1][j+3] = addr[i-1][j+3] + qeB * 1/16;
        addr[i-1][j+4] = addr[i-1][j+4] + qeG * 1/16;
        addr[i-1][j+5] = addr[i-1][j+5] + qeR * 1/16;
        if(addr[i-1][j+3] > 255){addr[i-1][j+3] = 255;}
        if(addr[i-1][j+3] < 0){addr[i-1][j+3] = 0;}
        if(addr[i-1][j+4] > 255){addr[i-1][j+4] = 255;}
        if(addr[i-1][j+4] < 0){addr[i-1][j+4] = 0;}
        if(addr[i-1][j+5] > 255){addr[i-1][j+5] = 255;}
        if(addr[i-1][j+5] < 0){addr[i-1][j+5] = 0;}
      }
    }
  }
*/
  ...
  char bqe1 = 0, bqe2 = 0, bqe3 = 0, bqe4 = 0;
  char gqe1 = 0, gqe2 = 0, gqe3 = 0, gqe4 = 0;
  char rqe1 = 0, rqe2 = 0, rqe3 = 0, rqe4 = 0;
  for (i = image->header.height - 1; i >= 0; i--){
    for (j = 0; j < image->header.width * 3; j = j+3){
      bqe1 = bqe2 = bqe3 = bqe4 = gqe1 = gqe2 = gqe3 = gqe4 = rqe1 = rqe2 = rqe3 = rqe4 = 0;
      if(i+1 <= image->header.height-1 && j-3 >= 0){
        bqe1 = (addr[i+1][j-3] - ((addr[i+1][j-3]/8) * 255 / 31));
        gqe1 = (addr[i+1][j-2] - ((addr[i+1][j-2]/8) * 255 / 31));
        rqe1 = (addr[i+1][j-1] - ((addr[i+1][j-1]/8) * 255 / 31));
      }
      if(i+1 <= image->header.height-1 && j+2 < image->header.width*3){
        bqe2 = (addr[i+1][j] - ((addr[i+1][j]/8) * 255 / 31));
        gqe2 = (addr[i+1][j+1] - ((addr[i+1][j+1]/8) * 255 / 31));
        rqe2 = (addr[i+1][j+2] - ((addr[i+1][j+2]/8) * 255 / 31));
      }
      if(i+1 <= image->header.height-1 && j+5 < image->header.width*3){
        bqe3 = (addr[i+1][j+3] - ((addr[i+1][j+3]/8) * 255 / 31));
        gqe3 = (addr[i+1][j+4] - ((addr[i+1][j+4]/8) * 255 / 31));
        rqe3 = (addr[i+1][j+5] - ((addr[i+1][j+5]/8) * 255 / 31));
      }
      if(j-3 >= 0){
        bqe4 = (addr[i][j-3] - ((addr[i][j-3]/8) * 255 / 31));
        gqe4 = (addr[i][j-2] - ((addr[i][j-2]/8) * 255 / 31));
        rqe4 = (addr[i][j-1] - ((addr[i][j-1]/8) * 255 / 31));
      }
      
      addr[i][j] = addCheck(addr[i][j], (bqe1 * 1 + bqe2 * 5 + bqe3 * 3 + bqe4 * 7) / 16);
      addr[i][j+1] = addCheck(addr[i][j+1], (gqe1 * 1 + gqe2 * 5 + gqe3 * 3 + gqe4 * 7) / 16);      
      addr[i][j+2] = addCheck(addr[i][j+2], (rqe1 * 1 + rqe2 * 5 + rqe3 * 3 + rqe4 * 7) / 16);
    }
  }
