...
/*****************************/
  ...
  for (i = image->header.height - 1; i >= 0; i--){
    for (j = 0; j < image->header.width * 3; j++){
      unsigned char oldColor = addr[i][j];
      unsigned char newColor = addr[i][j] / 8;
      addr[i][j] = newColor;
      unsigned char quant_err = oldColor - (newColor * 255 / 31);
      
      if (j+1 < image->header.width * 3){
        addr[i][j+1] = addr[i][j+1] + quant_err * 7/16;
        if(addr[i][j+1] > 255){addr[i][j+1] = 255;}
        if(addr[i][j+1] < 0){addr[i][j+1] = 0;}
      }
      if (i-1 >= 0 && j-1 >= 0){
        addr[i-1][j-1] = addr[i-1][j-1] + quant_err * 3/16; 
        if(addr[i-1][j-1] > 255){addr[i-1][j-1] = 255;}
        if(addr[i-1][j-1] < 0){addr[i-1][j-1] = 0;}
      }
      if (i-1 >= 0){
        addr[i-1][j] = addr[i-1][j] + quant_err * 5/16; 
        if(addr[i-1][j] > 255){addr[i-1][j] = 255;}
        if(addr[i-1][j] < 0){addr[i-1][j] = 0;}
      }
      if (i-1 >= 0 && j+1 < image->header.width * 3){
        addr[i-1][j+1] = addr[i-1][j+1] + quant_err * 1/16;
        if(addr[i-1][j+1] > 255){addr[i-1][j+1] = 255;}
        if(addr[i-1][j+1] < 0){addr[i-1][j+1] = 0;}
      }
    }
...
*/
  ...
  /*METHOD 2*/ /*
  unsigned char qe1, qe2, qe3, qe4;
  for (i = image->header.height - 1; i >= 0; i--){
    for (j = 0; j < image->header.width * 3; j++){
      qe1 = qe2 = qe3 = qe4 = 0;
      if(i+1 <= image->header.height-1 && j-1 >= 0){
        qe1 = addr[i+1][j-1] - ((addr[i+1][j-1]/8) * 255 / 31);
      }
      if(i+1 <= image->header.height-1){
        qe2 = addr[i+1][j] - ((addr[i+1][j]/8) * 255 / 31);
      }
      if(i+1 <= image->header.height-1 && j+1 < image->header.width*3){
        qe3 = addr[i+1][j+1] - ((addr[i+1][j+1]/8) * 255 / 31);
      }
      if(j-1 >= 0){
        qe4 = addr[i][j-1] - ((addr[i][j-1]/8) * 255 / 31);
      }
      addr[i][j] = addr[i][j] + (qe1 * 1 + qe2 * 5 + qe3 * 3 + qe4 * 7) / 16;
      if (addr[i][j] > 255){addr[i][j] = 255;}
      if (addr[i][j] < 0){addr[i][j] = 0;}
    }
  }
*/
  ...
  /*METHOD 3*/
  unsigned char **err;
  err = malloc(sizeof(char *) * image->header.height);
  for(i = 0; i < image->header.width * 3; i++){
    err[i] = malloc(sizeof(char) * image->header.width * 3);
  }
  ...
  for (i = image->header.height - 1; i >= 0; i--){
    for (j = 0; j < image->header.width * 3; j++){
      ...
      unsigned char quant_err = oldColor - (newColor * 255 / 31);
      if (j+1 < image->header.width * 3){
        err[i][j+1] = err[i][j+1] + quant_err * 7;
      }
      if (i-1 >= 0 && j-1 >= 0){
        err[i-1][j-1] = err[i-1][j-1] + quant_err * 3;
      }
      if (i-1 >= 0){
        err[i-1][j] = err[i-1][j] + quant_err * 5;
      }
      if (i-1 >= 0 && j+1 < image->header.width * 3){
        err[i-1][j+1] = err[i-1][j+1] + quant_err * 1;  
    }
  }
  for (i = 0; i < image->header.height; i++){
    for (j = 0; j < image->header.width * 3; j++){
      addr[i][j] = addr[i][j] + err[i][j] / 16;
    }
  }
  for(i = 0; i < image->header.width * 3; i++){
    free(err[i]);
